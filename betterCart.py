...
class betterCart:
	...
	max_x, max_v, max_t, max_w = 51, 41, 41, 63 #The max index for an obsevation
	...
	def updateQ(self, stateI, action, stateF):
		...
		reward = abs(ti) - abs(tf)

		self.ns[xi][vi][ti][wi][action]+=1
	...
	def convertToIndexs(self, state):
		...
		if x > 2.4:
			x = 2.5
		x = int(round(x*10 - .5) + (self.max_x)/2)
		if v <= -2:
			v = -2
		if v >= 2:
			v = 2
		v = int(round(v*10 - .5) + (self.max_v)/2)
		if t < -.2:
			t = -.2
		if t > .2:
			t = .2
		t = int(round(t*100 - .5) + (self.max_t)/2)
		if w <= -3:
			w = -3
		if w >= 3:
			w = 3
		w = int(round(w*10 - .5) + (self.max_w)/2)
		return x, v, t, w